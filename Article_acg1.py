...
def start():
    ...
    num = 10 #只爬取10张图,可以调大，如果中间网络异常会丢失几张图，最终数量可能达不到

    #创建B站专栏
    article = Article(cookies, "动漫-手机壁纸") #创建B站专栏草稿,并设置标题
    ...
    picList = session.get("http://service.picasso.adesk.com/v1/vertical/category/4e4d610cdf714d2966000003/vertical?limit=10&order=new").json()["res"]["vertical"]
    imageUrl = ''
    for i in range(num):
        url = picList[i]["img"]
        try:
    ...
    try:
        ...
        article.setListId(768787) #设置文集, 获取id列表请看详情
