...
def start():
    ...
    num = 8 #只爬取10张图,可以调大，如果中间网络异常会丢失几张图，最终数量可能达不到

    #创建B站专栏
    article = Article(cookies, "必应壁纸") #创建B站专栏草稿,并设置标题
    ...
    # n: 获取的数量(必须), 1-8
    # mkt: 地区(zh-CN)
    picList = session.get("https://cn.bing.com/HPImageArchive.aspx?format=js&idx=0&n=8&mkt=zh-CN").json()["res"]["vertical"]
    imageUrl = ''
    for i in range(num):
        url = picList[i]["img"]
        try:
