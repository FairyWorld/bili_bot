...
class Article(biliContext):
    ...
    def save(self):
        ...
        return self._api.createArticle(self._tilte, self._content.content, self._aid, self._category, self._list_id, self._tid, self._original, self._image_urls, self._origin_image_urls, False, self._tags)

    def submit(self):
        '''发布至B站上，必须先使用save方法保存到B站草稿箱'''
        return self._api.createArticle(self._tilte, self._content.content, self._aid, self._category, self._list_id, self._tid, self._original, self._image_urls, self._origin_image_urls, True, self._tags)
