...
def start():
    ...
    #创建B站专栏
    article = Article(cookies, "风景-手机壁纸") #创建B站专栏草稿,并设置标题
    ...
    picList = session.get("http://service.picasso.adesk.com/v1/vertical/category/4e4d610cdf714d2966000002/vertical?limit=10&order=new").json()["res"]["vertical"]
    imageUrl = ''
    for i in range(num):
        url = picList[i]["img"]
        try:
