...
def start():
    ...
    PHPSESSID = os.environ.get('SESSDATA', None) #必填，登录后获得的cookie
    
    if PHPSESSID and len(PHPSESSID):
        print('缺少PHPSESSID')
        return
    else:
        #创建B站专栏
        article = Article(cookies, "Pixiv美图") #创建B站专栏草稿,并设置标题
        ...
        for i in range(num):
            ...
            try:
                ...
                res = session.get(findurl, verify=False) #这里得到P站图片
